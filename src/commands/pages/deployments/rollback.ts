...
export async function run(args: string[], ctx: Context): Promise<void> {
  ...
  const deployment = await ctx.client.post<PagesDeployment>(
    `/accounts/${encodeURIComponent(accountId)}/pages/projects/${encodeURIComponent(project)}/deployments/${encodeURIComponent(id)}/rollback`,
  );

  ctx.output.success(`Rolled back project "${project}" to deployment "${id}" (new deployment: ${deployment.id}).`);
}
