...
export async function run(args: string[], ctx: Context): Promise<void> {
  ...
  // Filter to HTTP type policies (filters contains "http")
  const httpPolicies = policies.filter(
    (p) => p.filters?.includes("http") ?? false,
  );
  ...
  ctx.output.table(httpPolicies, columns);
}
