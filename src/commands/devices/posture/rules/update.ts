...
export async function run(args: string[], ctx: Context): Promise<void> {
  ...
  const rule = await ctx.client.put<DevicePostureRule>(
    `/accounts/${encodeURIComponent(accountId)}/devices/posture/${encodeURIComponent(id)}`,
    body,
  );

  ctx.output.success(`Posture rule "${rule.name}" updated.`);
  ctx.output.detail({
    "ID": rule.id,
    "Name": rule.name,
    "Type": rule.type,
