...
export async function run(args: string[], ctx: Context): Promise<void> {
  ...
  const result = await ctx.client.get<{ wans: Record<string, unknown>[] }>(
    `/accounts/${encodeURIComponent(accountId)}/magic/sites/${encodeURIComponent(site)}/wans`,
  );

  const wans = result.wans ?? [];
